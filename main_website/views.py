...
from wsgiref.util import FileWrapper# Create your views here.
...
def home_page(requests, company_name, stall_text):
    ...
    context = {
        ...
        'main_page_back_banner_image':organizer_details.main_page_front_banner_image,
    ...
    if(company_name == 'PSU'):
        return render(requests, 'new/main_page_new.html', context)
...
def lobby_page(requests, company_name, stall_text):
    ...
    if(company_name == 'PSU'):
        return render(requests, 'new/lobby_page_new.html', context)
...
def exhibit_hall(requests, company_name, stall_text):
    ...
    back_button = '/arena/lobby/' + company_name + '/' + stall_text + '/'
    back_button = urllib.parse.quote(back_button)
    stall_details = list(zip(stall_images, stall_texts))
    ...
    if(company_name == 'PSU'):
        return render(requests, 'new/exhibit_hall_new.html', context)
...
def stall_details(requests, company_name, stall_text):
    ...
    stall_contact_url = stall.stall_contact_url
    stall_schedule_url = stall.stall_schedule_url
    ...
    print("this is stall stall_contact_url",stall_contact_url)
    print("this is stall stall_schedule_url",stall_schedule_url)
    ...
    context = {
        ...
        'stall_contact_url':stall_contact_url,
        'stall_schedule_url':stall_schedule_url,
    ...
    return render(requests, 'new/stall_detail_jecrc.html', context)
...
def photobooth(request):
    if request.method == 'POST':
        base64image = request.POST.get("image", "").split(',',1)[1]
        msg = base64.b64decode(base64image)
        buf = io.BytesIO(msg)
        img = Image.open(buf)
        img.save('original.png')
        print('before save ++++++++++++')
        print(settings.BASE_DIR)
        foreground = Image.open(settings.BASE_DIR+'/main_website/static/new/photobooth/Photoframe.png')
        foreground = foreground.resize((img.width, img.height), Image.ANTIALIAS)
        ...
        img_str = "data:image/png;base64,"+base64.b64encode(myimage).decode()
...
def stall_analytics(request, company_name, stall_text):
    ...
    schedule_calls = call_schedules.objects.filter(schedule_id=stall_unique_id)
...
def first_floor(request):
    return render(request,"new/first_floor/first_floor.html")

def first_floor_link1(request):
    return render(request,"new/first_floor/first_floor_link1.html")        

def first_floor_link2(request):
    return render(request,"new/first_floor/first_floor_link2.html")

def first_floor_link3(request):
    return render(request,"new/first_floor/first_floor_link3.html")
...
def update_representative(request, company_name, stall_text):
    ...
    mobile = request.POST['mobile']
    designation = request.POST['designation']
    temp_data = Stall_Sales_Representatives.objects.get(stall_id = stall_text)
    temp_data.name = name
...
def schedule_call_by_user(request,company_name,stall_text):
    ...
    name,email=users.name,users.email
    call_schedules.objects.create(
                                    name=name,
                                    email=email,
                                    selected_date="-".join(start_date.split('/')[::-1]), #just for django
                                    selected_time=start_time

                                    )
    # print("he's the one",users)
    return HttpResponseRedirect(f"/arena/stall_details/{company_name}/{stall_text}/")
