...
def register(request, company_name, stall_text):
    ...
    organisation_unique_code = organization.unique_code
    unique_code_status=True
    ...
    if request.method == 'POST':
        ...
        stall_id = Stalls.objects.get(unique_url=stall_text, organization=organization.id)
        # inserted_user_code = request.data['unique_code']
        # if inserted_user_code is not organisation_unique_code:
        ...
        if registration_form.is_valid():
            ...
            if user_exist:
                ...
                inserted_user_code = request.POST['unique_code']
                print("Sending Back ")
                print(inserted_user_code)
                print(organisation_unique_code)
                print(type(inserted_user_code))
                print(type(organisation_unique_code))
                
                if inserted_user_code != organisation_unique_code:
                    unique_code_status=False
                    context = {
                        ...
                        'unique_code':organisation_unique_code,
...
def basic_login_redirect(request):
    organization = OrganizerDetails.objects.get(name='jecrc')
...
def send_welcome_mail(user_email, user_name):
    ...
    MESSAGE['FROM'] = "webinarwall772@gmail.com"
    body = """
            Dear {} <br>
            <h4>Greetings of the Day !!</h4>
            <b>Thank you for registering with Synnex Group for the exclusive live 3D “INDIA FOOD NUTRITION SUMMIT  2020” We are glad to welcome you on board.</b><br>
    ...
    server.starttls()
    server.ehlo()
    server.login('webinarwall772@gmail.com', 'qcxfarravsnzvyer')
    server.sendmail(
        'webinarwall772@gmail.com',
        user_email,
        MESSAGE.as_string()
    )
    server.quit()
